...
public:
    ...
    {
        auto handler = [viewHandler](const QHttpServerRequest &request, std::unique_ptr<QHttpServerResponder> responder) {
    ...
    {
        auto handler = [context, viewHandler](const QHttpServerRequest &request, std::unique_ptr<QHttpServerResponder> responder) {
            constexpr size_t count = ContextTypeForFunctor<ViewHandler>::arg_count;
    ...
    using Handler = std::function<void(const QHttpServerRequest &, std::unique_ptr<QHttpServerResponder>)>;
...
private:
    void handleRequest(const QHttpServerRequest &request, std::unique_ptr<QHttpServerResponder> responder) override;
