...
#include "include/qhttpserverresponse.h"
...
void QHttpServerResponse::setData(const QJsonObject &json)
{
    Q_D(QHttpServerResponse);

    d->data = QJsonDocument(json).toJson(QJsonDocument::Compact);
    setHeader(QHttpServerLiterals::contentTypeHeader(), QHttpServerLiterals::contentTypeJson());
}
...
void QHttpServerResponse::setStatusCode(const QHttpServerResponse::StatusCode code)
{
    Q_D(QHttpServerResponse);
    d->statusCode = code;
}
