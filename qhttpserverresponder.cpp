...
#include "include/qhttpserverresponder.h"
...
struct IOChunkedTransfer
{
        ...
        }))
    {
        ...
        QObject::connect(source, &QObject::destroyed, sink, &QObject::deleteLater);
                                 ...
                                 StatusCode status)
{
    ...
    Q_ASSERT(d->socket);
    std::unique_ptr<QIODevice, std::function<void(QIODevice*)>> input(data, [](QIODevice *p) {p->deleteLater();});
